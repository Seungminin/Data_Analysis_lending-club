...
def compute_return_mse(self, generated_data, real_return):
    fake_return = generated_data["expected_return"].values
    real_return = real_return.values if isinstance(real_return, pd.Series) else real_return
    mse_loss = F.mse_loss(torch.tensor(fake_return, dtype=torch.float32).to(self.device),
                          torch.tensor(real_return, dtype=torch.float32).to(self.device))
    return mse_loss
