...
class Generator(nn.Module):
    def __init__(self, input_dim, gside, num_channels):
        ...
        self.up1 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(num_channels * 8, num_channels * 4, 4, 2, 1),  # (B, C4, gside//4)
            nn.BatchNorm2d(num_channels * 4),
            nn.ReLU()
        )

        self.up2 = nn.Sequential(
            nn.ConvTranspose2d(num_channels * 4, num_channels * 2, 4, 2, 1),  # (B, C2, gside//2)
            nn.BatchNorm2d(num_channels * 2),
            nn.ReLU()
        )

        self.up3 = nn.Sequential(
            nn.ConvTranspose2d(num_channels * 2, num_channels, 4, 2, 1),  # (B, C, gside)
            nn.BatchNorm2d(num_channels),
            nn.ReLU()
        )

        self.out_conv = nn.Conv2d(num_channels, 1, kernel_size=3, padding=1)  # Output layer

        # Skip MLP for residual (optional)
        self.skip_fc = nn.Linear(input_dim, gside * gside)

    def forward(self, z):
        x = self.fc(z).view(-1, *self.init_dim)  # (B, C, H, W)

        u1 = self.up1(x)  # (B, C4, H*2, W*2)
        u2 = self.up2(u1)  # (B, C2, H*4, W*4)
        u3 = self.up3(u2)  # (B, C, H*8, W*8)

        out = self.out_conv(u3)  # (B, 1, gside, gside)

        # Optional skip-connection via latent vector
        skip = self.skip_fc(z).view(z.size(0), 1, self.init_dim[1]*8, self.init_dim[2]*8)
        return out + skip  # Residual connection
