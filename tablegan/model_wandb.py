...
Authors: Mahmoud Mohammadi, Noseong Park
Adopted from https://github.com/carpedm20/DCGAN-tensorflow
Created : 07/20/2017
Modified: 10/15/2018
Modified further to include wandb logging.
"""
...
class TableGan(object):
    def __init__(self, sess, input_height=108, input_width=108, crop=True,
        ...
        # Batch normalization layers (help gradient flow)
        self.d_bn1 = batch_norm(name='d_bn1')
        self.d_bn2 = batch_norm(name='d_bn2')

        # Classifier normalization layers
        ...
        self.alpha = alpha  # Info Loss Weight
        self.beta = beta    # Class Loss Weight
    ...
    def build_model(self):
        ...
        if self.y_dim:
            ...
            # Classifier: optionally mask duplicated label columns
            if self.label_col > 0:
        ...
        self.info_loss = tf.add(
            tf.maximum(0.0, tf.reduce_sum(tf.abs(self.gmean - self.gmean_) - self.delta_mean)),
            tf.maximum(0.0, tf.reduce_sum(tf.abs(self.gvar - self.gvar_) - self.delta_var))
        )
    ...
    def train(self, config, experiment):
        ...
        # Initialize wandb with configuration parameters
        wandb.init(project="Lending_club for tablegan", config={
        ...
        DIS_SKIP_COUNT = 1     # 그중 1개 에폭은 D를 학습 안 함
        ...
        for epoch in xrange(config.epoch):
            ...
            freeze_d = (epoch % DIS_UPDATE_EVERY) > DIS_SKIP_COUNT  # ✅ Discriminator 주기적 휴식 적용 (4번 째 휴식)
            ...
            for idx in xrange(batch_idxs - 1):
                ...
                if self.y_dim:
                    wandb.log({"c_loss": errC, "step": counter, "epoch": epoch})
                    experiment.log_metric("c_loss", errC, step=idx)
            ...
            avg_g_loss = np.mean(g_losses_in_epoch)
            wandb.log({"average_g_loss": avg_g_loss, "step": counter, "epoch": epoch})
            ...
            print("\n🔍 Avg g_loss for epoch {}: {:.6f}, Best so far: {:.6f}, Patience: {}".format(
                epoch, avg_g_loss, best_g_loss, patience))

            if patience >= patience_threshold:
                print("\n🚨 Early stopping triggered after epoch {} with avg g_loss: {:.6f}".format(epoch, avg_g_loss))
                return
    ...
    def classification(self, image, y, reuse=False):
        with tf.variable_scope("classification") as scope:
            ...
            yb = tf.reshape(y, [self.batch_size, 1, 1, self.y_dim])
            x = conv_cond_concat(image, yb)
            h0 = lrelu(conv2d(x, self.c_dim + self.y_dim, name='c_h0_conv'))
            h0 = conv_cond_concat(h0, yb)
            h1 = lrelu(self.c_bn1(conv2d(h0, self.df_dim + self.y_dim, name='c_h1_conv')))
            h1 = tf.reshape(h1, [self.batch_size, -1])
            h1 = concat([h1, y], 1)
    ...
    def generator(self, z, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            if self.y_dim:
                h0 = conv_cond_concat(h0, yb)
            h2, self.h2_w, self.h2_b = deconv2d(h0, [self.batch_size, s_h4, s_w4, self.gf_dim * 2], name='g_h2', with_w=True)
